...
public class ChangeNicerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (youNicer.getAdmin() != Nicer.isAdmin && youNicer.getId() != id) {
				req.getSession().setAttribute("error", "你没有管理员权限，请用管理员账号登录！");
			...
			if (nicer != null) {
				...
				req.setAttribute("error", "学号为：" + idString + "的账号不存在！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			req.setAttribute("error", "学号为：" + idString + "的账号不存在！");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			HttpSession session = req.getSession();
			Nicer youNicer = (Nicer) session.getAttribute("youNicer");
			if (youNicer.getAdmin() != Nicer.isAdmin && map.get("id").equals(youNicer.getId() + "")) {
				jsonObject.put("result", false);
				jsonObject.put("info", "你没有管理员权限，请用管理员账号登录！");
